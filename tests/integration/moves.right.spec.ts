...
describe('integration right moves', () => {
  it('horizontal merges right with spawn', () => {
    const s = init({ spawn_script: createSpawnScript([{ row: 1, col: 1, value: 2 }]) });
    s.board = [[2,0,2,0],[0,0,2,0],[2,0,0,0],[2,0,2,0]];
    const next = move(s, 'Right');
    expect(next.board).toEqual([[0,0,0,4],[0,0,0,4],[0,0,0,2],[0,0,0,4]]);
  });
});
